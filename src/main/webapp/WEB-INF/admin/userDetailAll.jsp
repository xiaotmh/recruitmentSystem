...
<head>
    ...
    <%
    List<UserDetail> userdetails  = (List<UserDetail>)request.getAttribute("lists");
                        ...
                        <div class="card">
                            ...
                            <div class="card-body">
                                <button type="button" class="btn btn-primary" data-toggle="modal" data-target="#addManager">
                                    <i class="mdi mdi-account-plus"></i>&nbsp;<strong>新增用户信息</strong>
                                </button>
                                <br>
                                <form  method="post">
                                    <table id="managerTable" class="table table-hover" >
                                        ...
                                        %>
                                        <tr class="warning">
                                            <td ><input class="text " value="<%=userdetails.get(i).getUid() %>" id="a<%=i%>" style="width:60px;" readonly="true" ></td>
                                            <td ><input class="text " value="<%=userdetails.get(i).getRealname() %>"  id="b<%=i%>" style="width:80px;" ></td>
                                            <td ><input class="text " value="<%=userdetails.get(i).getAge() %>"  id="c<%=i%>" style="width:80px;" ></td>
                                            <td ><input class="text " value="<%=userdetails.get(i).getGender() %>"  id="d<%=i%>" style="width:80px;" ></td>
                                            <td ><input class="text " value="<%=userdetails.get(i).getEmail() %>"  id="e<%=i%>" style="width:120px;" ></td>
                                            <td ><input class="text " value="<%=userdetails.get(i).getPhone() %>"  id="f<%=i%>" style="width:80px;" ></td>
                                            <td ><input class="text " value="<%=userdetails.get(i).getQq() %>"  id="g<%=i%>" style="width:80px;" ></td>
                                            <td ><input class="text " value="<%=userdetails.get(i).getMajor() %>"  id="h<%=i%>" style="width:80px;" ></td>
                                            <td ><input class="text " value="<%=userdetails.get(i).getClassname() %>"  id="i<%=i%>" style="width:80px;" ></td>
                                            <td ><input class="text " value="<%=userdetails.get(i).getEdubackground() %>"  id="j<%=i%>" style="width:80px;" ></td>
                                            <td>
                                                <button type="button" class="btn btn-warning" onclick="window.location.href='userdetailupdate?uid='+$('#a<%=i%>').val()+ '&&realname='+$('#b<%=i%>').val()
                                                        + '&&age='+$('#c<%=i%>').val()+ '&&gender='+$('#d<%=i%>').val()+ '&&email='+$('#e<%=i%>').val()+ '&&phone='+$('#f<%=i%>').val()
                                                        + '&&qq='+$('#g<%=i%>').val()+ '&&major='+$('#h<%=i%>').val()+ '&&classname='+$('#i<%=i%>').val()+ '&&edubackground='+$('#j<%=i%>').val()" >提交修改</button>
                                                <button type="button" class="btn btn-danger" onclick="window.location.href='userdetaildelete?uid=<%=userdetails.get(i).getUid()%>'">删除</button>
                                            </td>
                                                <%
                                            }
                                        %>
                                    ...
                                    <ul class="gcs-pagination">
                                        ...
                                        <li><a href="alluserdetail2?pageNum=1">首页</a></li>
                                        <%
                                            if(pageNum!=1){
                                        %>
                                        <li><a href="alluserdetail2?pageNum=<%=pageNum-1%>">上一页</a></li>
                                        <%
                                            }
                                        %>
                                        <li><a href="alluserdetail2?pageNum=<%=pageNum%>">当前页数:<%=pageNum%></a></li>
                                        <li><a href="#">总页数:<%=count%></a></li>
                                        <%
                                            if(pageNum!=count){
                                        %>
                                        <li><a href="alluserdetail2?pageNum=<%=pageNum+1%>">下一页</a></li>
                                        <%
                                            }
                                        %>
                                        <li><a href="alluserdetail2?pageNum=<%=count%>">>尾页</a></li>
                                    ...
                                    <div class="modal-dialog" role="document">
                                        <div class="modal-content">
                                            <div class="modal-header">
                                                ...
                                                <h4 class="modal-title" id="exampleModalLabel"><i class="mdi mdi-account-plus"></i>&nbsp;<strong>新增用户</strong></h4>
                                            </div>
                                            <%--uid, realname,age,gender,email,phone,qq,major,classname,edubackground--%>
                                            <div class="modal-body">
                                                <form id="newManager" action="userdetailinsert" method="get">
                                                    ...
                                                    <div class="form-group">
                                                        ...
                                                        <input type="text" class="form-control" name="edubackground" id="edubackground">
                                                    </div>
                                                    <div class="modal-footer">
                                                        <button type="button" class="btn btn-default" data-dismiss="modal">关闭</button>
                                                        <button type="button" class="btn btn-primary" onclick="window.location.href='userdetailinsert?uid='+$('#uid').val()+'&&realname='
                                                                +$('#realname').val()+'&&age='
                                                                +$('#age').val()+'&&realname='
                                                                +$('#uid').val()+'&&gender='
                                                                +$('#gender').val()+'&&email='
                                                                ...
                                                                +$('#qq').val()+'&&major='
                                                                +$('#major').val()+'&&classname='
                                                                +$('#classname').val()+'&&edubackground='
                                                                +$('#edubackground').val()">确定</button>
                                                    </div>
                                                </form>
                                            </div>
