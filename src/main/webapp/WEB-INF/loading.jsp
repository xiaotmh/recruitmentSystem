		...
		<div class="container">
			<div class=" row">
				...
				<div class="col-sm-3" style="border-right:1px dotted #d4d3d3;border-radius: 10px;">
					...
					<div id="pop22" class="div112Intro"><img src="images/intro-1.gif" align="middle" /><span class="span12InnerIntro">工作经验</span></div>
					<div id="pop23" class="div112Intro"><img src="images/intro-1.gif" align="middle" /><span class="span12InnerIntro">教育经历</span></div>
					<div id="pop24" class="div112Intro"><img src="images/intro-1.gif" align="middle" /><span class="span12InnerIntro">培训经历</span></div>
					<div id="pop25" class="div112Intro"><img src="images/intro-1.gif" align="middle" /><span class="span12InnerIntro">语言能力</span></div>
					<div id="pop26" class="div112Intro"><img src="images/intro-1.gif" align="middle" /><span class="span12InnerIntro">求职意向/自我评价</span></div>
					...
					<div id="pop10" class="div112Intro"><img src="images/intro-1.gif" align="middle" /><span class="span12InnerIntro">高级人才附加信息</span></div>
					<div id="pop11" class="div112Intro"><img src="images/intro-1.gif" align="middle" /><span class="span12InnerIntro">IT技能</span></div>
					<div id="pop12" class="div112Intro"><img src="images/intro-1.gif" align="middle" /><span class="span12InnerIntro">项目经验</span></div>
				...
				<%
					...
					List<EtuserDetail> EDlist=(List<EtuserDetail>) request.getSession().getAttribute("EDlist");
					int pageNum=(Integer) request.getAttribute("page");
					int count = (Integer)request.getAttribute("count");
					if(list.size()>0){
					for (Resume resume : list) {
						for (EtuserDetail etd : EDlist) {
							if(etd.getBid()==resume.getEnid()){
				%>
				<!--中间内容右边开始-->
				<div class="col-sm-9">
					...
					<div >
						...
						<div class="row">
							<div class="col-sm-6">
								<%--<h4 class="media-heading">企业id:<small style="margin-left: 10px;"><%=resume.getEnid()%></small>
									<small style="margin-left: 10px;"><a class="btn-warning">已投递</a></small>
								</h4>--%>
								<h4 class="media-heading"><%--企业id:<small style="margin-left: 10px;"><%=resume.getEnid()%></small>--%>
									企业名：<small style="margin-left: 10px;"><%=etd.getRealname()%></small>
									<br>
									<small style="margin-left: 10px;"><a class="btn-warning">已投递</a></small>
								</h4>
							</div>
							<div class="col-sm-4">
								<a class="btn-sm btn-warning" onclick="window.location.href='myreply?uid=<%=resume.getUid()%>&&enid=<%=resume.getEnid()%>&&status=<%=resume.getStatus()%>'">查看老板回复</a>
							</div>
						</div>
						<br>
				...
				<ul class="gcs-pagination">
					...
					<li><a href="userresume2?pageNum=1&&uid=<%=list.get(0).getUid()%>">首页</a></li>
					<%
						if(pageNum!=1){
					%>
					<li><a href="userresume2?pageNum=<%=pageNum-1%>&&uid=<%=list.get(0).getUid()%>">上一页</a></li>
					<%
						}
					%>
					<li><a href="userresume2?pageNum=<%=pageNum%>&&uid=<%=list.get(0).getUid()%>">当前页数:<%=pageNum%></a></li>
					<li><a href="#">总页数:<%=count%></a></li>
					<%
						if(pageNum!=count){
					%>
					<li><a href="userresume2?pageNum=<%=pageNum+1%>&&uid=<%=list.get(0).getUid()%>">下一页</a></li>
					<%
						}
					%>
					<li><a href="userresume2?pageNum=<%=count%>&&uid=<%=list.get(0).getUid()%>">>尾页</a></li>
                 <%
					 }else{
                 %>
					<center>这里还空空如也QAQ</center>
					<%
						}
					%>
				</ul>
